...
The interface is Swedish by default. Use the visible **Svenska / English** language toggle to switch languages; the selection is saved in `localStorage` and restored on the next visit. The lesson covers the C4–C5 range (C4 middle C through the C5 above the treble staff). The keyboard includes touch-friendly black keys for C♯4/D♭4, D♯4/E♭4, F♯4/G♭4, G♯4/A♭4, and A♯4/B♭4, with labels and selection highlighting. The quiz shows the same full keyboard so it looks like a real piano, but the questions stay white-note-only so learners practise the eight pitches shown on the staff; tapping a black key plays its pitch and points the learner back to the staff instead of advancing the round. The microphone practice mode is also focused on the C4–C5 range and works best when you play a real piano close to the device microphone in a quiet room.
